...
<html lang="pt-br">
	<head>
		<meta charset="utf-8">
		<title>dRAD | Dimensão Ensino</title>
		<link type="text/css" rel="stylesheet" media="screen" href="CSS/style1.css"/>
	</head><br>
	<?php echo utf8_decode($row1['titulo']) . " ". utf8_decode($row1['nome']);?> | <a href="ensino.php">Voltar</a></br></br><hr>
		<div class="container" >
			<div class="content">
				<div id="vip" style="margin:auto auto" align=center>
					<h2>1.27 - Coorientação de TCC de cursos de especialização lato sensu DENTRO da carga horária contratual, incluindo a modalidade EAD e/ou programas de residência na UPE.</h2>
						<form action="" name="CadastroAlunos" type="text" method="post" enctype="multipart/form-data">
							<table>
								...
								<tr><input type="text" name="nome"  placeholder="Nome do Discente"/></tr>
								<tr><input type="text" name="nome1"  placeholder="Título do Trabalho"/></tr>
								<tr><input type="text" name="nome2"  placeholder="Departamento ou Programa"/></tr>
							</table>
							<table width="400px">	
								<tr width="400px">
						...
						<?php // onclick="window.open('upload4.php', '_blank');"  
							...
							{								
								if(($_FILES['image']['size'] == 0) || (empty($_POST['ano'])) || (empty($_POST['semestre'])))
								{
									...
									$ano = $_POST['ano'];
									$n1 = $_POST['nome'];
									$nome = '<b>Disciplina:</b>' . '  ' . $n1;
									$n2 = $_POST['nome1'];
									$nome1 = '<b>Título:</b>' . '  ' . $n2;
									$n3 = $_POST['nome2'];
									$nome2 = '<b>Departamento/Programa:</b>' . '  ' . $n3;
									...
									if(!preg_match('/^image\/(?:jpg|jpeg|png)$/i', $_FILES['image']['type'])){;
										...
										$sql_code ="INSERT INTO banco (iduser, item, ano, semestre, nome, nome1, nome2, pdf, arquivo) VALUES('$iduser', '$item', '$ano', '$semestre', '$nome', '$nome1', '$nome2', '$image', '$novo_nome')";
										...
										$sql_code ="INSERT INTO banco (iduser, item, ano, semestre, nome, nome1, nome2, foto, arquivo) VALUES('$iduser', '$item', '$ano', '$semestre', '$nome', '$nome1', '$nome2', '$image', '$novo_nome')";
										$varX=mysqli_query($conexao, $sql_code);										
										if(move_uploaded_file($_FILES['image']['tmp_name'],$diretorio. $novo_nome)){
											$msg="Arquivo carregado com sucesso!";
										}else{
								   ...
								   ?><table><br><hr>
										...
										<td align="left"><?php 
											...
											<td><?php 
													...
													$var2 = utf8_encode(utf8_decode($row["nome1"]));
													$var3 = utf8_encode(utf8_decode($row["nome2"]));
													$var4 = $row["ano"];
													$var5 = $row["semestre"];
													$varZ = sprintf("%s<br> %s<br> %s<br> <b>Ano: </b>%s<br> <b>Semestre: </b>%s", $var1, $var2, $var3, $var4, $var5);
													echo $varZ;
