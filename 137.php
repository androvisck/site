		...
		<div class="container" >
			<div class="content">
				<div id="vip" style="margin:auto auto" align=center>
					<h2>1.37 - Coordenação/regência de disciplina, componente curricular ou módulo (apenas, quando houver mais de dois professores por disciplina, componente curricular ou módulo).</h2>
						<form action="" name="CadastroAlunos" type="text" method="post" enctype="multipart/form-data">
							<table>
								...
								<tr><input type="text" name="nome"  placeholder="Nome da Disciplina, Componente ou Módulo"/></tr>
								<tr><input type="text" name="nome1"  placeholder="Departamento ou Programa"/></tr>
							</table>
							<table width="400px">	
								<tr width="400px">
						...
						<?php // onclick="window.open('upload4.php', '_blank');"  
							...
							{								
								if(($_FILES['image']['size'] == 0) || (empty($_POST['ano'])) || (empty($_POST['semestre'])))
								{
									...
									$ano = $_POST['ano'];
									$n1 = $_POST['nome'];
									$nome = '<b>Disciplina/Componente/Módulo:</b>' . '  ' . $n1;
									$n2 = $_POST['nome1'];
									$nome1 = '<b>Departamento/Programa:</b>' . '  ' . $n2;
									...
									if(!preg_match('/^image\/(?:jpg|jpeg|png)$/i', $_FILES['image']['type'])){;
										...
										$sql_code ="INSERT INTO banco (iduser, item, ano, semestre, nome, nome1, pdf, arquivo) VALUES('$iduser', '$item', '$ano', '$semestre', '$nome', '$nome1', '$image', '$novo_nome')";
										...
										$sql_code ="INSERT INTO banco (iduser, item, ano, semestre, nome, nome1, foto, arquivo) VALUES('$iduser', '$item', '$ano', '$semestre', '$nome', '$nome1', '$image', '$novo_nome')";
										$varX=mysqli_query($conexao, $sql_code);										
										if(move_uploaded_file($_FILES['image']['tmp_name'],$diretorio. $novo_nome)){
											$msg="Arquivo carregado com sucesso!";
										}else{
								   ...
								   ?><table><br><hr>
										...
										<td align="left"><?php 
											...
											<td><?php 
													...
													$var3 = $row["ano"];
													$var4 = $row["semestre"];
													$varZ = sprintf("%s<br> %s<br> <b>Ano: </b>%s<br> <b>Semestre: </b>%s", $var1, $var2, $var3, $var4);
													echo $varZ;
