					...
					<form method="post" action="" ></br>
						...
						<?php
							...
							{															
								...
								$result = "SELECT * FROM admin WHERE matricula LIKE '%".$matricula."%' ORDER BY id DESC";
								...
								while($row=mysqli_fetch_array($res)){
								   ?><table cellspacing="10"><br><hr>
										<tr> 
											...
											<td align="left"><?php
												$var1 = $row["id"];
												$var2 = utf8_encode($row["nome"]);
												$var3 = utf8_encode($row["sobrenome"]);
												$var4 = $row["matricula"];
												$var5 = utf8_encode($row["titulo"]);
												$var6 = utf8_encode($row["lotacao"]);
												$var7 = $row["login"];
										...
										<?php echo "<a href='edit_admin.php?matricula=" . $row['matricula'] . "'>Editar</a>"; ?>
										<?php echo " | "; ?>				
										<?php echo "<a href='apag_admin.php?matricula=" . $row['matricula'] . "' onclick=\"return confirm('Deseja mesmo apagar o registro?');\">Apagar</a>"; ?>
										</td></tr></th>
										<br>
									</table>
								   <?php
